...
// 注入 VIBETERM_E2E_GATEWAY_PORT。
const DEFAULT_GATEWAY_PORT = 9665;

// 兜底防线：webServer 启动早于 globalSetup（Playwright 保证），此处在任何用例运行前
// 直接探测被测 gateway 的 healthz，断言它是 NODE_ENV=test 实例。一旦误连到生产 VibeTerm
...
export default async function globalSetup(_config: FullConfig): Promise<void> {
  ...
  // mesh 的三个实例由 tests/helpers/relay-boot.ts 自己在空闲端口上拉起，端口不会撞生产。
  if (process.env.VIBETERM_E2E_MESH_ONLY === '1') return;
  ...
  if (body.env !== 'test') {
    throw new Error(
      ...
      }，期望 'test'。这可能是生产 VibeTerm（9883）。请用 \`bun run test:e2e\`，或显式设置 VIBETERM_E2E_GATEWAY_PORT / VIBETERM_E2E_FE_PORT。`
    );
  }
}
