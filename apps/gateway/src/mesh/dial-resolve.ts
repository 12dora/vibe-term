...
export type DialWsFactoryDeps = {
  wsCtor?: DialWsCtor;
  resolve?: (host: string) => Promise<DialResolveResult | null>;
  enabled?: boolean;
  raceCount?: number;
};
...
export function dialTlsForHost(host: string, baseTls?: DialTlsBase | null): DialSocketOpts {
  return {
    tls: { ...baseTls, serverName: host },
    headers: { host },
  };
}
...
export async function resolveDialHost(
  ...
  const work = resolveUncached(hostname, opts).then((result) => {
    remember(hostname, result, now());
    noteTransition(hostname, result);
    return result;
  });
...
export function createDialWsFactory(
  ...
  const resolve = deps.resolve ?? ((hostname) => resolveDialHost(hostname));
  const baseTls = tlsCa && tlsCa.length > 0 ? { ca: tlsCa } : undefined;
  return async (url, ctx) => {
    const host = hostOfDialUrl(url) ?? '';
    const open = (target: string) => wsCtor(target, dialTlsForHost(host, baseTls));
    try {
      return await raceOrOpen(open, url, ctx, deps.raceCount);
    } catch (err) {
      const ipUrl = await fallbackDialUrl(url, host, err, resolve, deps.enabled);
      if (!ipUrl) throw err;
      return await raceOrOpen(open, ipUrl, ctx, deps.raceCount);
    }
  };
}
...
export async function fetchWithDnsFallback(
  ...
  try {
    ...
    const ipUrl = await fallbackDialUrl(
      ...
      host,
      err,
      opts.resolve ?? resolveDialHost,
      opts.enabled
    );
    if (!ipUrl || !host) throw err;
    const dial = dialTlsForHost(host, (init as { tls?: DialTlsBase }).tls);
    return await doFetch(ipUrl, {
      ...init,
      tls: dial.tls,
      headers: { ...plainHeaders(init.headers), host },
    } as RequestInit);
  }
}
...
async function fallbackDialUrl(
  url: string,
  host: string | null,
  err: unknown,
  resolve: (hostname: string) => Promise<DialResolveResult | null>,
  enabled?: boolean
): Promise<string | null> {
...
function isNonFallbackFailure(err: unknown): boolean {
  ...
  return /\beconnrefused\b|connection refused/.test(blob);
}
...
async function resolveUncached(
  ...
  const systemIp = pickDialIp(
    await lookupTimed(lookup, hostname, opts.timeoutMs ?? DIAL_SYSTEM_LOOKUP_TIMEOUT_MS)
  );
  if (systemIp) return { ip: systemIp, via: 'system' };
  if (!canUseDoh(opts)) return null;
  ...
  try {
    ...
    return dohIp ? { ip: dohIp, via: 'doh' } : null;
  } catch {
    return null;
  }
}
...
async function lookupTimed(lookup: DialLookup, hostname: string, ms: number): Promise<string[]> {
...
function noteTransition(host: string, result: DialResolveResult | null): void {
  if (!result) return;
