...
- 「中继管理」tab：瓦片分「流量 / 进程」两组，「租户」与「放行带宽」两格在配了上限时显示 `已用 / 上限`；趋势卡三条 5 分钟折线（吞吐、活跃流、事件循环延迟），成员表（RTT、流、速率、重连、接入时间）。未挂任何中继时 `/api/mesh/relay/status` 返回 `mode: "none"`（`RelayUplinkMode` = `'relay' | 'none'`），前端按角色而非该字段决定文案。
