...
- 只有内网地址的云主机（1:1 DNAT）在 peer server 绑定全部接口时，额外追加一条 `ws://<公网 IPv4 或主机名>:<peer 端口>/peer`。公网地址优先取 `VIBETERM_PEER_PUBLIC_HOST`（可广告的公网 IPv4 或 FQDN，非法值启动时警告并忽略），否则取 STUN 探测 `ok` 且 mapped 本身可广告的地址（只用 IP）。**不要**用探针行上的 `fakeIp` 否决 mapped：`fakeIp` 只表示解析 STUN **服务器域名**时系统 DNS 见过 `198.18/15`，与 Binding 回报的映射地址无关；TUN 代理机上几乎每条探针都是 `fakeIp: true`，否决它会把真实公网出口丢光。多条 STUN 报出不同公网 IP 时按 mapped IPv4 计票：仅 1 条有效样本则采用；≥2 条时取票数 ≥2 且严格过半的那个，平票或无多数则宁可不广告，也不要广告代理分流出口。永不追加 fake-IP `198.18/15`、CGNAT、RFC1918；映射地址变化时随 status 重新广播。对端会 TCP 探测并可能直连这条公网地址。
