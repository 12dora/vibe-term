...
节点加入 mesh 后，入口 hub 的公开域名会把流量转发到各节点；同时节点自己也可能被配置成公开域名（tunnel、反代、自签公开地址）。有些机器只希望在局域网里被人用，公网域名只用来跑 mesh 自身的服务流量。每个节点因此有一个开关：关掉之后，**经公开域名进来的用户流量**被拒，局域网 IP / localhost / `.local` 与 mesh 服务路由照常。
