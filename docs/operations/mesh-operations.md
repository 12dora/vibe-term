...
| 变量 | 默认 | 说明 |
|---|---|---|
| `VIBETERM_PEER_BIND_HOST` | 未设 | peer 口绑定。空 / 未设 → dual-stack `::` 与 `0.0.0.0`。由 `config.ts` 的 `parsePeerBindHost` 读取 |
| `VIBETERM_PEER_PUBLIC_HOST` | 未设 | 1:1 DNAT / 仅内网网卡的节点要广播的公网 IPv4 或 FQDN。未设时回落 STUN mapped 的多数派 IPv4。必须是可广告公网 IPv4（拒绝 fake-IP / CGNAT / RFC1918）或含点号的主机名；非法值启动时 `console.warn` 后当作未设。`init` / `upgrade` 不写入，需手改 `app.env` |
...
本机卡「网络 → 端口」行按角色列出端口计划（`relay` / `relay,node` 含 443 与 TURN，其余为 peer + RTC），标签固定为「端口」，不再按角色换标题，也没有图例行；节点详情框仍用「入站端口」。三态灯：`open` 绿、`blocked` 红、`unknown` 或无 `MeshPortReach` 行灰点，灯的 `title` / `aria-label` 即「已开通 / 未开通 / 未探测」。探测中保持原灯 + 按钮 spinner。能解析到 self id（`mode.nodeId` 或 mesh `entryNodeId`）时行末显示「重新检测」，打 `POST /api/mesh/nodes/<self>/ports/probe`；standalone / 无 self 不画该按钮。灯表示「别人探本机 advertised peer endpoint」，不是安全组扫描。只有内网地址的云主机会额外广播 STUN mapped 多数派公网 IPv4（或 `VIBETERM_PEER_PUBLIC_HOST`），所以安全组已放行 39001 的云主机也能变绿；`refused` 一次即红，`timeout` 两击。「重新检测」让对端 30 s 内重探（经 `peer_reach_epoch`）。中继主机的 443 与 TURN 行由成员在线数与成员 TURN 统计派生。
...
**v1 支持的平台：** macOS arm64 / x64，Linux glibc x64 / arm64。**musl、Windows、其它 arch 不支持**（`lookupNativePin` 返回 `null`，enable 失败且不阻断）。缺失或装载失败 → `direct_capable=false`。authorize 在没有 RTC provider 时回 503 `DIRECT_UNAVAILABLE` `reason=disabled`，原生栈没就绪回 `reason=native-missing`。这两档客户端停放 10 分钟，只有用户重试才再问。指纹超时、中止、内部失败、名额满回 `DIRECT_BUSY`（带 `retryAfterMs`），走熔断，不停放。名额是每会话 2、每用户 8、全节点 64。
...
| 503 `DIRECT_UNAVAILABLE` `reason=disabled` / `native-missing` | 直连关掉，或原生栈没装上 | `direct enable`；看 `VIBETERM_NATIVE_DIR` 与 `native/manifest.json`。客户端停放 10 分钟，页面刷新不会再问，要用户点重试。装不了的平台接受 relay |
| 503 `DIRECT_BUSY` | 这次没协商出来：`reason=timeout` / `aborted` / `failed` / `capacity`，带 `retryAfterMs` | 不是「这台不能直连」。`capacity` 是名额满（每会话 2、每用户 8、全节点 64），等 `retryAfterMs` 或关掉多余标签。`timeout` 看目标日志有没有 `authorize failed`。老节点把这些原因塞进 `DIRECT_UNAVAILABLE` 时，新客户端仍按忙处理 |
| 经入口打开终端正常，但直连徽标一直不亮，authorize 约 5–7 s 后 503 `NODE_UNREACHABLE` `reason=timeout` | 2.8.0 及更早的目标在空 PC 上等指纹，入口转发预算先到，目标日志是空的 | 把**目标**升到含探测通道授权的版本。升完后目标应有 `[mesh][rtc] authorize`，失败才有 `authorize failed … reason=`。见 [KI-19](../known-issues.md) |
...
| `[mesh][rtc] dial failed` 刷屏、中继入站 UDP 被滤 | 对 `direct_capable≠false` 的 peer 会反复拨 DC | 连续 3 次**拨号**失败后打 `[mesh][rtc] breaker trip peer=… fails=… level=… cooldown_ms=… until=…`，冷却期内跳过该 peer 的 DC（起始 30 s，`VIBETERM_RTC_DIAL_BREAKER_MS` 可改起始档；之后 60 s / 120 s … 上限 30 min）。跳过时 `dial failed` 带 `cause=breaker_cooling`，同一 peer 60 s 至多一条并带 `count=`。健康满 60 s 才 `[mesh][rtc] breaker reset` 并清零，`disabled` 也要满这 60 s 才解除。中继会话闪断**不**清零。离开 ≥ 90 s 再上线只降一档（`breaker rearm … source=presence-return`）。能力 / 版本 / endpoint / 本机指纹 / 换上行是整档清零。对端报了 `dc-stable-hold` 时，建立后 60 s 内夭折走 `unstable-dc` 冷却（60 s 起、翻倍、封顶 30 min），不抬拨号档位。细节见 [节点直连](../architecture/peer-direct-connect.md) |
| `datachannel closed … initiator= reason= lifetime_ms= proven=` 成串，寿命约 10 s | 通道刚开就被掐掉。用来区分「原生 / 对端关」和「本端 liveness 关」 | `initiator=remote` 且 `reason=channel-closed` 是对端或原生栈关的；`reason=liveness-timeout` 且 `initiator=local` 是本端 10 s 无入站。同一事件另有 `dc drop … proven= lifetime_ms=`。`proven=false` 时中继不该已经拆掉。对端没报 `dc-stable-hold` 时短命断开仍计拨号失败 |
| `signal dropped kind=sdp cause=fake-ip` | 外发 SDP 里有 `198.18.0.0/15` 的 `a=candidate`，已剥掉 | 本机 TUN 假 IP。trickle 候选的 `signal dropped … cause=fake-ip` 是另一条。局域网地址不会被这行丢掉 |
